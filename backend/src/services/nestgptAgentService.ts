...
const agentToolsConfig = {
  functionDeclarations: [
    {
      name: 'ask_question',
      description: 'Ask the user a question to gather more information about their campaign. Use this when you need specific details like brand name, target audience, campaign goals, budget, channels, etc.',
      parameters: {
        type: 'OBJECT',
        properties: {
          ...
          category: { 
            type: 'STRING', 
            enum: ['brand_info', 'campaign_goal', 'target_audience', 'budget', 'channels', 'timeline', 'tone', 'content_type'],
            description: 'The category of information being collected'
          }
        },
    ...
    },
    {
      ...
      description: 'Analyze current social media analytics data to inform campaign strategy. Returns engagement metrics, best performing content, and audience insights.',
      parameters: {
        type: 'OBJECT',
        properties: {
          platforms: { type: 'ARRAY', items: { type: 'STRING' }, description: 'Platforms to analyze' },
          timeframe: { type: 'STRING', enum: ['7d', '14d', '30d'], description: 'Timeframe for analytics' }
        },
        required: ['platforms']
      }
    },
    {
      ...
      description: 'Generate a campaign image on the canvas. Use this when you have enough information to create visual content for the campaign.',
      parameters: {
        type: 'OBJECT',
        properties: {
          imagePrompt: { type: 'STRING', description: 'Detailed prompt for the image generation' },
          canvasName: { type: 'STRING', description: 'Name for the canvas' },
          aspectRatio: { type: 'STRING', enum: ['1:1', '16:9', '9:16'], description: 'Aspect ratio for the image' },
          brandName: { type: 'STRING', description: 'Brand name for context' }
        },
    ...
    },
    {
      ...
      description: 'Create a comprehensive marketing plan based on collected information. Use this after gathering sufficient details about the campaign.',
      parameters: {
        type: 'OBJECT',
        properties: {
          ...
          channels: { type: 'ARRAY', items: { type: 'STRING' }, description: 'Social media channels to use' },
          budget: { type: 'STRING', description: 'Budget level or amount' },
    ...
    },
    {
      ...
      description: 'Generate a content calendar with specific posts for each day of the campaign.',
      parameters: {
        type: 'OBJECT',
        properties: {
          ...
          campaignTheme: { type: 'STRING', description: 'Overall campaign theme' }
        },
    ...
    },
    {
      ...
      parameters: {
        type: 'OBJECT',
        properties: {
          campaignData: { type: 'OBJECT', description: 'Complete campaign data to save' }
        },
...
const getAnalyticsData = async (platforms: string[], timeframe: string = '14d') => {
  return {
    summary: {
    ...
    platformBreakdown: {
      ...
      twitter: {
        ...
        avgRetweets: 45,
        avgLikes: 180,
        topContentType: 'threads',
        bestPostingTime: '9am-10am'
      },
      linkedin: {
        followers: 5600,
        avgLikes: 120,
...
const generateContentCalendar = async (params: any) => {
  ...
  const contentTypes = [
    { type: 'Educational', description: 'Tips, how-tos, industry insights' },
    { type: 'Promotional', description: 'Product/service highlights' },
    { type: 'Engagement', description: 'Questions, polls, user-generated content' },
    { type: 'Behind-the-scenes', description: 'Team, process, culture' },
    { type: 'Social Proof', description: 'Testimonials, case studies, reviews' }
  ];
  ...
  for (let week = 1; week <= durationWeeks; week++) {
    ...
    for (const day of daysToPost) {
      ...
      weekPosts.push({
        ...
        day: ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][postDate.getDay()],
        channel,
        contentType: contentType.type,
        suggestion: `${contentType.type} post about ${campaignTheme}`,
        time: channel === 'linkedin' ? '9:00 AM' : channel === 'instagram' ? '12:00 PM' : '10:00 AM'
      });
    }
...
class NestGptAgentService {
  ...
  async processMessage(sessionId: string | undefined, userMessage: string): Promise<{
    reply: string;
    sessionId: string;
    thoughts: string[];
    ...
    const hasSubstantialInfo = session.collectedInfo.brandName && 
                               session.collectedInfo.campaignGoal && 
                               session.collectedInfo.targetAudience;
    // Require MINIMUM 3 questions, allow up to 5 maximum
    const shouldForceAction = questionsAskedCount >= 5 || (questionsAskedCount >= 3 && hasSubstantialInfo);

    thoughts.push(`📋 Session state: ${questionsAskedCount} questions asked, has substantial info: ${hasSubstantialInfo}, force action: ${shouldForceAction}`);
    ...
    const model = genAI.getGenerativeModel({
      ...
      toolConfig: shouldForceAction ? {
        functionCallingConfig: {
          ...
          allowedFunctionNames: ['create_marketing_plan', 'generate_campaign_image', 'generate_content_calendar', 'analyze_analytics', 'save_campaign']
        }
      } : undefined,
      generationConfig: {
        temperature: 0.7,
    ...
    try {
      ...
      const chat = model.startChat({
        history: [
          ...
          { role: 'model', parts: [{ text: 'Understood. I\'m ready to help create a comprehensive marketing campaign. I\'ll gather information step by step and use my tools to analyze, create visuals, and build a complete plan.' }] },
      ...
      if (functionCalls && functionCalls.length > 0) {
        for (const functionCall of functionCalls) {
          ...
          // Execute the tool
          const toolResult = await this.executeTool(session, toolName, args, thoughts, actions);
          ...
          if (toolName === 'ask_question') {
            ...
            thoughts.push('📊 Analyzed social media performance data');
            ...
            session.createdCanvases.push(toolResult.canvas?.id || '');
          } else if (toolName === 'create_marketing_plan') {
            thoughts.push('📋 Generated comprehensive marketing plan');
            session.campaignPlan = toolResult;
            session.phase = 'planning';
            actions.push({ type: 'marketingPlan', data: toolResult });
          } else if (toolName === 'generate_content_calendar') {
            thoughts.push('📅 Created content calendar');
            actions.push({ type: 'contentCalendar', data: toolResult });
          } else if (toolName === 'save_campaign') {
            thoughts.push('💾 Campaign saved successfully');
            session.phase = 'complete';
            actions.push({ type: 'campaignSaved', data: toolResult });
          }
          ...
          if (toolName !== 'ask_question') {
            const followUp = await chat.sendMessage([{
              functionResponse: {
                name: toolName,
                response: { result: JSON.stringify(toolResult).slice(0, 1000) }
              }
            }]);
            ...
            if (!finalReply || finalReply.trim() === '') {
              finalReply = await this.generateContextualReply(session, toolName, toolResult);
            }
          }
        }
      } else {
        ...
        if (!textResponse || textResponse.trim() === '') {
          textResponse = await this.generatePosterDescription(session, userMessage);
        }
      ...
      const errorReply = 'I apologize, I encountered an issue processing your request. Could you please try again?';
  ...
  private buildSystemPrompt(session: AgentSession): string {
    ...
    const questionsAskedCount = session.questionsAsked.length;
    const hasEnoughInfo = questionsAskedCount >= 5 || (questionsAskedCount >= 3 && session.collectedInfo.brandName && session.collectedInfo.campaignGoal && session.collectedInfo.targetAudience);
...
- No channels specified? Assume Instagram, Twitter, LinkedIn
...
Be friendly, professional, and guide the user step by step.`;
  ...
  private getMissingInfo(session: AgentSession): string[] {
    ...
    if (!info.channels || info.channels.length === 0) missing.push('social channels');
  ...
  private async executeTool(
    ...
    switch (toolName) {
      ...
      case 'analyze_analytics':
        const analytics = await getAnalyticsData(args.platforms || ['instagram', 'twitter', 'linkedin'], args.timeframe);
        return analytics;
      ...
      case 'create_marketing_plan':
        const plan = await createMarketingPlan({
          ...
          targetAudience: args.targetAudience || session.collectedInfo.targetAudience,
  ...
  private extractAndUpdateInfo(session: AgentSession, message: string): void {
    ...
    if (!session.collectedInfo.brandName) {
      ...
      if (quotedMatch) {
        ...
        const brandMatch = message.match(/\b([A-Z][a-zA-Z0-9]+(?:\s+[A-Z][a-zA-Z0-9]+)?)\b/);
        if (brandMatch && !['I', 'The', 'A', 'We', 'My', 'Our', 'Hi', 'Hello', 'Hey'].includes(brandMatch[1])) {
    ...
    if (lowerMessage.includes('instagram') || lowerMessage.includes('insta')) channels.push('instagram');
    if (lowerMessage.includes('twitter') || lowerMessage.includes(' x ') || lowerMessage.includes('tweet')) channels.push('twitter');
    if (lowerMessage.includes('linkedin')) channels.push('linkedin');
    if (lowerMessage.includes('facebook') || lowerMessage.includes(' fb ')) channels.push('facebook');
    if (lowerMessage.includes('tiktok') || lowerMessage.includes('tik tok')) channels.push('tiktok');
    if (channels.length > 0) {
      session.collectedInfo.channels = channels;
    }

    // Goal detection - more patterns
    if (lowerMessage.includes('awareness') || lowerMessage.includes('visibility') || lowerMessage.includes('reach')) {
      session.collectedInfo.campaignGoal = 'brand awareness';
    } else if (lowerMessage.includes('launch') || lowerMessage.includes('new product') || lowerMessage.includes('releasing')) {
      session.collectedInfo.campaignGoal = 'product launch';
    } else if (lowerMessage.includes('engagement') || lowerMessage.includes('followers') || lowerMessage.includes('community')) {
      session.collectedInfo.campaignGoal = 'increase engagement';
    } else if (lowerMessage.includes('sales') || lowerMessage.includes('conversion') || lowerMessage.includes('revenue') || lowerMessage.includes('sell')) {
      session.collectedInfo.campaignGoal = 'drive conversions';
    } else if (lowerMessage.includes('promote') || lowerMessage.includes('marketing') || lowerMessage.includes('campaign')) {
      session.collectedInfo.campaignGoal = 'promotion';
    }

    // Budget detection
    if (lowerMessage.includes('low budget') || lowerMessage.includes('limited') || lowerMessage.includes('small budget') || lowerMessage.includes('tight budget')) {
      session.collectedInfo.budget = 'low';
    } else if (lowerMessage.includes('medium') || lowerMessage.includes('moderate') || lowerMessage.includes('reasonable')) {
      session.collectedInfo.budget = 'medium';
    } else if (lowerMessage.includes('high budget') || lowerMessage.includes('large budget') || lowerMessage.includes('big budget') || lowerMessage.includes('unlimited')) {
  ...
  private updatePhase(session: AgentSession): void {
    ...
    const hasDetailedInfo = hasBasicInfo && info.targetAudience && info.channels;

    if (session.campaignPlan) {
      session.phase = session.createdCanvases.length > 0 ? 'complete' : 'creating';
  ...
  private async generateContextualReply(session: AgentSession, toolName: string, toolResult: any): Promise<string> {
  ...
  private async generatePosterDescription(session: AgentSession, userMessage: string): Promise<string> {
    const model = genAI.getGenerativeModel({ 
      model: 'gemini-2.5-flash',
    ...
    const audience = session.collectedInfo.targetAudience || 'general audience';
...
Would you like me to create this visual in the Canvas Studio? I can generate multiple variations in different formats (square for Instagram, landscape for Twitter, etc.)`;
  }
