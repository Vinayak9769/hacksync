...
import { StickyNote, Type, Trash2, Send, Move, Users, MoreHorizontal } from "lucide-react"
...
const initialNotes: Note[] = [
  ...
  { id: "2", type: "sticky", content: "User testimonial video series", x: 350, y: 150, color: colorOptions[1] },
  { id: "3", type: "sticky", content: "Weekly tips thread on Twitter", x: 600, y: 100, color: colorOptions[2] },
  { id: "4", type: "text", content: "Q1 2026 Content Strategy", x: 250, y: 350, color: "bg-transparent" },
  { id: "5", type: "sticky", content: "Partner with micro-influencers", x: 500, y: 300, color: colorOptions[3] },
]
...
export default function WhiteboardPage() {
  ...
  const addNote = (type: "sticky" | "text") => {
    const newNote: Note = {
      ...
      color: type === "sticky" ? colorOptions[Math.floor(Math.random() * colorOptions.length)] : "bg-transparent",
    }
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    if (draggedNote) {
      ...
      setNotes(notes.map((n) => (n.id === draggedNote ? { ...n, x: Math.max(0, newX), y: Math.max(0, newY) } : n)))
    }
  }
    ...
    <div className="p-6 h-[calc(100vh-3.5rem)] flex flex-col">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">Brainstorm and collaborate on content ideas</p>
        </div>
        ...
        <CardContent className="p-0 h-full">
          ...
          >
            {notes.map((note) => (
              <div
                key={note.id}
                className={cn("absolute cursor-move select-none", selectedNote === note.id && "ring-2 ring-primary")}
